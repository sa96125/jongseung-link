...
원래라면 프로그래머가 모든일을 다 해야했지만, 프레임워크에게 개발자가 처리해야할 일을 역임시키고 개발자는 비지니스로직에만 집중하는 것 제어의 주체가 역전되었음을 의미합니다. 이게 프레임워크의 핵심 역할이고 리액트보다 유연하고, 재사용성이 높아지는 넥스트를 사용해야하는 가장 큰 이유라고 생각합니다. 정말 유명한 말이 있죠. "내가 할 수 있다고 해서 내가 해야하는 것은 아니다"
...
기능적인 측면에서 왜 Next.js가 좋은지 알아보겠습니다. 각 기능에 대한 정의는 공식홈페이지에 튜토리얼을 진행하시면 더 쉽고 빠르게 이해할 수 있으니, 저는 실제 사례 또는 예시코드를 가지고 성능적인 부분을 다뤄보겠습니다.
...
정리하면, 라우팅을 위해 더 이상 React-Router와 같은 라이브러리는 필요없다! Next.js의 내장 라우팅 시스템만으로 충분히 기능적인 라우팅을 구현할 수 있다!라고 말할 수 있습니다. 사용해보면 알겠지만, 페이지와 URL 경로 간의 일관성을 유지할 수 있기때문에 생산성이 정말 높아지고 실수가 확연히 줄어듭니다. 개인적으로 넥스트 사용에 있어 가장 만족스러운 부분이었습니다!
